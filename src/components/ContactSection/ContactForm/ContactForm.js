...
const ContactForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { response, loading } = await fetchData(
  ...
  return (
    ...
    >
      <StyledContactForm method="POST" onSubmit={handleSubmit}>
        ...
        <FormFooter>
          {response.mssg !== null && (
            <Response $status={response.status}>{response.mssg}</Response>
          )}
          {loading && (
            <ThreeDot variant="bounce" color={`#${colors.blue}`} size="small" />
          )}
          {!loading && (
            <Button
              text={"send"}
